...
namespace Tangra.Model.Numerical
{
    ...
    {
        public static double CalculateCriticalValue(double df, double confidenceLevel, double confidenceLevelError)
        {
            double t = 1;
            double calcConfidence = 0;
            do
            {
                calcConfidence = Student(t, df);
                t += (calcConfidence - confidenceLevel);
            }
            while (Math.Abs(calcConfidence - confidenceLevel) > confidenceLevelError);
